...
//  pins.
#define ESP32_GPIO2    2 // Blue LED.
#define ESP32_GPIO4    4 // Relay control.
#define ESP32_GPIO5    5 // Optocoupler input.

#define RelayPIN ESP32_GPIO4
#define InputPIN ESP32_GPIO5
#define LED_PIN ESP32_GPIO2
...
WebServer *server;   //Web server object. Will be listening in port 80 (default for HTTP)
...
void callback(char* topic, byte* payload, unsigned int length) {
	...
	if (strPacket != "")
	{
		...
		// Ensure we don't overflow the mqtt_msg buffer
		size_t msgLen = strPacket.length() + 1; // +1 for null terminator
		if (msgLen > sizeof(mqtt_msg)) {
			msgLen = sizeof(mqtt_msg);
		}
		strPacket.toCharArray(mqtt_msg, msgLen);
		delay(100);
...
void InitServer()
{
	server = new WebServer(storage.nHttpPort);
	...
	else if (strPrefix == "Mode")
	{
		...
		else
		{
			if (strPass == DevPass)
			{
				...
				size_t msgLen = strtemp.length() + 1;
				if (msgLen <= sizeof(mqtt_msg)) {
					strtemp.toCharArray(mqtt_msg, msgLen);
					if (client.connected())
						client.publish(mqtt_Out, mqtt_msg);
				}					}
					if (storage.P2 == OVR)
	...
	if (StartupTON.Q)
	{
		...
		case 1:// Normal Mode
		{
			if (nTimeMs < millis() && !RelayPermant)
				relayState = false;
			// If timer hasn't expired or permanent mode, keep previous state
			// (state is set by OpenRelay/CloseRelay commands)
...
void ResponsetoCommand(String message)
{
	...
	if (msgLen <= sizeof(mqtt_msg)) {
		message.toCharArray(mqtt_msg, msgLen);
		if (client.connected())
			client.publish(mqtt_Out, mqtt_msg);
	}
...
String RefreshPage(String msg)
{
	...
	HtmlContent += "<title>ESP32 WIFI Relay Control Page</title>";
	...
	HtmlContent += "<div class = 'header'><h3 class = 'text-muted'>ESP32 WIFI Relay Control</h3>";
...
String MainPageNoneAuth(String msg)
{
    ...
    <title>ESP32 WIFI Relay Control Page</title>\
